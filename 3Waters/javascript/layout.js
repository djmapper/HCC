...
/* ================ custom code by Rhys Donoghue at Eagle Technology Group Ltd ======================================= */
   ...
   if (configOptions.displayfullextent === 'true' || configOptions.displayfullextent === true) {
    var btnZoomToFullExtent = new dijit.form.Button({
      ...
      dojo.byId('webmap-toolbar-center').appendChild(btnZoomToFullExtent.domNode);
    }
    ...
    if(configOptions.displayclearselection === 'true' || configOptions.displayclearselection === true) {
      ...
      dojo.byId('webmap-toolbar-center').appendChild(btnClearMap.domNode);
    }
...
/* ================ custom code by Rhys Donoghue at Eagle Technology Group Ltd ======================================= */
 ...
 function initSearch() {
        ...
        findTask = new esri.tasks.FindTask("http://gisviewer.hcc.govt.nz/arcgis/rest/services/AGOL_Property/PropertyInformation/MapServer");
        findTaskCemetery = new esri.tasks.FindTask("http://gisviewer.hcc.govt.nz/arcgis/rest/services/AGOL_Cemetery/Cemetery/MapServer");
...
function showResults(results) {
 ...
 try {
	...
	//made colour change using named color "yellow"	
        var polygonSymbol = new esri.symbol.SimpleFillSymbol(esri.symbol.SimpleFillSymbol.STYLE_NULL, new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new dojo.Color("yellow"), 3), new dojo.Color([255,255,0,0.25]));
        var lineSymbol = new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new dojo.Color("yellow"),2);
        ...
        var items = dojo.map(results,function(result){
          ...
          return result.feature.attributes;
           ...
           $('#clickOnMapTip').css({ visibility: 'hidden' });
...
function updateTypeAhead() {
 ...
 $("#searchField").autocomplete({
	            source: function (request, response) {
	                $.ajax({
	                    url: "http://gisviewer.hcc.govt.nz/WebServices/TypeAhead/searchJSON.ashx",
	                    dataType: "jsonp",
	                    data: {
